...
DumpResult<void> dumpProperties(fs::FilePageWriter& writer, PropertyContainer* container) {
    switch (container->getValueType()) {
        ...
        case ValueType::DateTime: {
            TrivialPropertyContainerDumper<types::DateTime> dumper(writer);
            if (auto res = dumper.dump(container->cast<types::DateTime>()); !res) {
                return res.get_unexpected();
            }
            break;
        }
        case ValueType::Map: {
            MapPropertyContainerDumper dumper(writer);
            if (auto res = dumper.dump(container->cast<types::Map>()); !res) {
                return res.get_unexpected();
            }
            break;
        }
