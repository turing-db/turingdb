...
MapPropertyContainerDumper::MapPropertyContainerDumper(fs::FilePageWriter& writer)
    : _writer(writer)
{
}

MapPropertyContainerDumper::~MapPropertyContainerDumper() {
}

DumpResult<void> MapPropertyContainerDumper::dump(const TypedPropertyContainer<types::Map>& props) {
    Profile profile("MapPropertyContainerDumper::dump");

    const uint64_t propCount = props.size();

    // A map has no fixed width, so the values go out as one byte stream of
    // [length][encoded map] records rather than as a strided array. The stream is built
    // up front because its total size is part of the metadata page, which is written
    // first.
    std::vector<std::byte> stream;
    for (const MapView map : props.all()) {
        const EncodedMap encoded(map);
    ...
    _writer.writeToCurrentPage(ValueType::Map);
