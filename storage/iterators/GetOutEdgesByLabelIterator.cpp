#include "GetOutEdgesByLabelIterator.h"
...
GetOutEdgesByLabelChunkWriter::GetOutEdgesByLabelChunkWriter(const GraphView& view,
...
void GetOutEdgesByLabelChunkWriter::filterTombstones() {
...
void GetOutEdgesByLabelChunkWriter::fill(size_t maxCount) {
    ...
    // The edges kept here are the ones arriving at a label set, and the edge index is
    // keyed by the source's, so there is no run to walk: each edge of the span is kept on
    // its own target's labels, as GetOutEdgesByTypeChunkWriter keeps it on its own type.
