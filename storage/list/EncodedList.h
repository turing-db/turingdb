...
// The encoding is canonical over the tags and payloads a list holds, so two encodings match
// byte for byte when they hold the same list. It is the structural equality the values a
// write buffer carries are compared by, not the numeric one operator==(ListView, ListView)
// gives Cypher: an integer and a double holding one number encode differently.
bool operator==(const EncodedList& lhs, const EncodedList& rhs);
