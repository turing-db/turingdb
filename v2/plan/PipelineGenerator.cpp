...
void PipelineGenerator::generate() {
    ...
    for (const auto& node : rootNodes) {
        ...
        nodeStack.push({node, nullptr, MaterializeProcessor::create(_pipeline, _mem)});
    }
    ...
    while (!nodeStack.empty()) {
        ...
        auto processNextNode = [&](PlanGraphNode* nextNode) {
            ...
            if (!nextNode->isBinary()) {
                nodeStack.push({nextNode, outputIf, newMatProc});
            } else {
                // Binary node case
                ...
                if (visited) {
                    ...
                    nodeStack.push({nextNode, outputIf, nullptr});
                    return;
                }
