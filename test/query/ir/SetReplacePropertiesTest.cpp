...
class SetReplacePropertiesTest : public TuringTest {
protected:
...
TEST_F(SetReplacePropertiesTest, replacesThePropertiesWithAMap) {
    write("MATCH (n:Person {name: 'Adam'}) SET n = {name: 'Adam2'}");
    expectRows("MATCH (n:Person {name: 'Adam2'}) RETURN n.age, n.dob", {{"null", "null"}});
}

TEST_F(SetReplacePropertiesTest, keepsTheEntriesOfTheMap) {
    write("MATCH (n:Person {name: 'Remy'}) SET n = {name: 'Remy', age: 40}");
    expectRows("MATCH (n:Person {name: 'Remy'}) RETURN n.age, n.dob", {{"40", "null"}});
}
