...
class FuseScanOutEdgesByLabelCodegenTest : public TuringTest {
public:
...
protected:
    ...
    // The program opens on one by-label edge scan and no node scan or hop is left.
...
TEST_F(FuseScanOutEdgesByLabelCodegenTest, bothEndpointsOfALabelledHopBecomeAnEdgeScan) {
...
TEST_F(FuseScanOutEdgesByLabelCodegenTest, targetOnlyProjectionBecomesAnEdgeScan) {
    const mlir::OwningOpRef<mlir::ModuleOp> module = generate("MATCH (a:Person)-->(b) RETURN b");

    expectFusedToLabelledEdgeScan(*module);
}

TEST_F(FuseScanOutEdgesByLabelCodegenTest, twoLabelsRideOntoTheEdgeScan) {
...
TEST_F(FuseScanOutEdgesByLabelCodegenTest, sourcePropertyIsReadOffTheEdgeScan) {
    ...
    EXPECT_EQ(properties.front().getInputNodes(), edgeScans.front().getSrcids());
}

// An in-hop reads the in-edge index, so it compiles to the in-edge scan instead.
TEST_F(FuseScanOutEdgesByLabelCodegenTest, labelledInHopBecomesTheInEdgeScan) {
    const mlir::OwningOpRef<mlir::ModuleOp> module = generate("MATCH (a:Person)<--(b) RETURN a, b");
...
TEST_F(FuseScanOutEdgesByLabelCodegenTest, labelledUndirectedHopKeepsItsNodeScan) {
    const mlir::OwningOpRef<mlir::ModuleOp> module = generate("MATCH (a:Person)--(b) RETURN a, b");
...
TEST_F(FuseScanOutEdgesByLabelCodegenTest, typedLabelledHopKeepsItsByTypeHop) {
...
TEST_F(FuseScanOutEdgesByLabelCodegenTest, unlabelledHopStillBecomesAWholeEdgeScan) {
...
TEST_F(FuseScanOutEdgesByLabelCodegenTest, twoHopChainFusesOnlyItsFirstHop) {
...
// The predicate sinks onto a's scan and fuses into a property scan, which is no longer the
// by-label scan the edge scan replaces.
TEST_F(FuseScanOutEdgesByLabelCodegenTest, sourcePredicateSinksToTheScanAndKeepsIt) {
