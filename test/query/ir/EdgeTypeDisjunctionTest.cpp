...
TEST_F(EdgeTypeDisjunctionTest, WritePatternStillRejectsMoreThanOneType) {
    NullSink sink;
    const QueryStatus status = runQuery("CREATE (a:Person)-[:KNOWS_WELL|INTERESTED_IN]->(b:Person)", &sink);
