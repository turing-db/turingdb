...
class UnwoundMapPropertyTest : public TuringTest {
protected:
...
TEST_F(UnwoundMapPropertyTest, createsNodesHoldingTheUnwoundMaps) {
    write("UNWIND [{a: 1}, {a: 2}] AS m CREATE (:Tagged {attrs: m})");
    expectRows("MATCH (n:Tagged) WHERE n.attrs = {a: 2} RETURN count(n)", {{"1"}});
}

TEST_F(UnwoundMapPropertyTest, setsAnUnwoundMap) {
    write("MATCH (n:Person {name: 'Remy'}) UNWIND [{a: 1}] AS m SET n.attrs = m");
    expectRows("MATCH (n:Person) WHERE n.attrs = {a: 1} RETURN n.name", {{"Remy"}});
}

TEST_F(UnwoundMapPropertyTest, createsANodeHoldingAnIndexedMap) {
    write("CREATE (:Tagged {attrs: [{a: 1}][0]})");
