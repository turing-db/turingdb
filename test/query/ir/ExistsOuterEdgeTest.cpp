...
class ExistsOuterEdgeTest : public TuringTest {
protected:
...
TEST_F(ExistsOuterEdgeTest, matchesAnOuterEdgeInsideExists) {
    expectRows("MATCH ()-[e]->() WHERE EXISTS { ()-[e]->(:Person) } RETURN e.name",
               {{"Remy -> Adam"}, {"Adam -> Remy"}, {"Ghosts -> Remy"}});
}

TEST_F(ExistsOuterEdgeTest, matchesAnOuterEdgeBetweenBoundEndpointsInsideExists) {
    expectRows("MATCH (a)-[e]->(b) WHERE EXISTS { (a)-[e]->(:Person) } RETURN e.name",
               {{"Remy -> Adam"}, {"Adam -> Remy"}, {"Ghosts -> Remy"}});
}

TEST_F(ExistsOuterEdgeTest, matchesAnOuterEdgeInsideNotExists) {
    expectRows("MATCH (a:Person)-[e]->() WHERE NOT EXISTS { ()-[e]->(:Interest) } RETURN e.name",
               {{"Remy -> Adam"}, {"Adam -> Remy"}});
}
