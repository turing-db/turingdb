...
TEST_F(GetPropertiesProcessorTest, test) {
    auto [transaction, view, reader] = readGraph();

    const PropertyType ageType = view.metadata().propTypes().get("age").value();

    LineContainer<NodeID, int64_t, EdgeID, NodeID, EdgeTypeID> expLines;
    LineContainer<NodeID, int64_t, EdgeID, NodeID, EdgeTypeID> resLines;
    ...
    const auto callback = [&](const Dataframe* df, LambdaProcessor::Operation operation) -> void {
        ...
        const ColumnVector<int64_t>* ages = df->getColumn<ColumnVector<int64_t>>(agesTag);
