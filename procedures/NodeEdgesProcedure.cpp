...
ProcedureData* NodeEdgesProcedure::allocData() {
    return new Data();
}

void NodeEdgesProcedure::deallocData(ProcedureData* data) {
    delete data;
}

void NodeEdgesProcedure::registerProcedure(ProcedureNamespace* ns) {
    Procedure* proc = new Procedure("nodeEdges");
...
void NodeEdgesProcedure::execute(ProcedureState* proc) {
    ...
    switch (proc->getStep()) {
        case ProcedureState::Step::PREPARE: {
            bioassert(rawInputCol, "db.nodeEdges: must be provided a nodeID column");
            ...
            bioassert(containerKind == ContainerKind::code<ColumnVector<void>>(),
                      "db.nodeEdges: nodeID input must be a vector column");
