...
# A split-Parquet export is a directory holding nodes.parquet + edges.parquet.
# Place it inside the data directory so LOAD PARQUET can resolve it by name.
mkdir -p $SCRIPT_DIR/.turing/data/typed
cp $SCRIPT_DIR/nodes.parquet $SCRIPT_DIR/.turing/data/typed/
cp $SCRIPT_DIR/edges.parquet $SCRIPT_DIR/.turing/data/typed/
