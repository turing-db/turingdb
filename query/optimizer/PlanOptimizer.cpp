...
static Column* buildTypedValuesColumn(LocalMemory* mem,
    ...
    for (const auto& [decl, propName, expr] : updates) {
        auto* lit = static_cast<const LitT*>(
            static_cast<const LiteralExpr*>(expr)->getLiteral());
...
static EvaluatedType literalKindToEvaluatedType(Literal::Kind kind) {
    switch (kind) {
        ...
        case Literal::Kind::STRING:    return EvaluatedType::String;
        case Literal::Kind::BOOL:      return EvaluatedType::Bool;
...
void PlanOptimizer::rewriteConstWriteSources() {
    ...
    for (PlanGraphNode* node : snapshot) {
        auto* writeNode = dynamic_cast<WriteNode*>(node);
        if (!writeNode) {
            continue;
        }
        ...
        if (!writeNode->pendingNodes().empty() || !writeNode->pendingEdges().empty()
            || !writeNode->toDeleteNodes().empty() || !writeNode->toDeleteEdges().empty()
        ...
        const auto firstKind = static_cast<const LiteralExpr*>(
            updates[0]._propValueExpr)->getLiteral()->getKind();
        for (const auto& update : updates) {
            const auto* litExpr = static_cast<const LiteralExpr*>(update._propValueExpr);
        ...
        auto* flatNodeIDs = _mem->alloc<ColumnNodeIDs>();
        for (const auto& update : updates) {
            const auto* srcCol = dynamic_cast<const ColumnNodeIDs*>(
                varToScan.at(update._decl)->values());
        ...
        VarDecl* nodeIDDecl = VarDecl::create(_ast, optDeclCtx, *nidName,
        ...
        auto* cwsNode = _plan->create<ConstWriteSourceNode>(flatNodeIDs, flatValues,
                                                             nodeIDDecl, valuesDecl);
