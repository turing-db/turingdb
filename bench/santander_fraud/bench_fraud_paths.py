#!/usr/bin/env python3
"""
Benchmark variable-length path queries on a synthetic bank-fraud graph.
...
Queries run through the turingdb shell, and the median of the timed runs is reported,
the first discarded as a warmup.
...
def selectQueries(groups, only):
    selected = []
    for group, queryID, question, query in QUERIES:
        if group not in groups:
            continue
        if only and queryID not in only:
            continue
        selected.append((group, queryID, question, query))
    return selected


def buildScript(queries, graph, reps, quiet):
    lines = [f"load graph {graph}", f"cd {graph}"]
    if quiet:
        lines.append("quiet")

    for _, queryID, _, query in queries:
...
def report(queries, results):
    for group, title in GROUPS.items():
        ...
        print(f"\n{group}. {title}")
        print("-" * 82)
        print(f"  {'question':58} {'ms':>9} {'rows':>11}")

        for _, queryID, question, _ in inGroup:
            time = median(results, queryID)
            rows = countOrRows(results.get(queryID))

            if time is None:
                error = (results.get(queryID) or {}).get("error") or "FAILED"
                print(f"  {question:58} {error.split(':')[0]:>9}")
                continue

            print(f"  {question:58} {time:9.2f} {rows!s:>11}")
...
def main():
    ...
    parser.add_argument("-timeout", type=float, default=5400, help="seconds for the whole run")
    ...
    aggregates = [q for q in queries if "count(" in q[3]]

    script = buildScript(queries, args.graph, args.reps, True)
    output = runShell(args.binary, args.turingDir, args.port, script, args.timeout)
    results = parseShellOutput(output)

    if args.verify and aggregates:
        valueScript = buildScript(aggregates, args.graph, 1, False)
        valueOutput = runShell(args.binary, args.turingDir, args.port + 1, valueScript, args.timeout)

        for queryID, result in parseShellOutput(valueOutput).items():
            if queryID in results:
                results[queryID]["value"] = result["value"]
