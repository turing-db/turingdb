module {
  func.func @main() {
    ...
    // (team) as the grouping key; aggregates [2, 3] names the two reductions over
    // the trailing columns (2 = min, 3 = max), so the op takes one key column and
    // two aggregate-input columns (both the score column) and emits one row per
    // team: the team, its lowest score, and its highest score.
    //
    ...
    %gteam, %lo, %hi = db.group_aggregate(%team, %score, %score) keys 1 aggregates [2, 3] : (!db.column<none>, !db.column<none>, !db.column<none>) -> (!db.column<none>, !db.column<none>, !db.column<none>)
