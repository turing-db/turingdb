module {
  func.func @main() {
    ...
    //
    // db.scan_edges_by_type opens the dataflow with the edges of the named
    // types - source node, edge, edge type and target node - the same four
    // columns db.scan_edges exposes, narrowed to those types. A pattern may
    // name several, as -[:KNOWS_WELL|INTERESTED_IN]-> does. The whole-graph node
