module {
  func.func @main() {
    ...
    %a = db.scan_nodes() : !db.column<!storage.nodeid>

    // First hop a->b, nothing carried yet so the carry set is `{}`.
    %a1, %e0, %et0, %b = db.get_out_edges(%a, {}) : (!db.column<!storage.nodeid>) -> (!db.column<none>, !db.column<none>, !db.column<none>, !db.column<none>)
